 ...
 */
function set_focus_on(type)
{
    var inputObj = $('.item_sel:eq('+window.showIndex+')').find('input:eq('+window.inputIndex+')');
    inputObj.focus();
    ...
    if (inputObj.attr('type') === 'checkbox') {
        var left = $('.h_left').scrollLeft() - offset;
        $('.h_left').scrollLeft(left);
    }
}
...
function reset_work_area_size()
{
    ...
    $('#main .content').css({'height': (viewHeight - 250) + 'px'});
}
...
{
    return window.perSelWidth * window.showIndex;
}
 ...
 * 滚动
 */
function show_animate(type)
{
    var left = 0;
    if (type === '+') {
        left = $('.h_left').scrollLeft() + window.perSelWidth;
    } else if (type === '-') {
        left = $('.h_left').scrollLeft() - window.perSelWidth;
    }
    if (left <= 0) {
